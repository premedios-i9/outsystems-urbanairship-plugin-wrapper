...
module.exports = function (context) {
    return new Promise(function (resolve, reject) {
        ...
        if (!val) {
            ...
            if (!zipFile) {
                return reject(
                    "Failed to install UrbanAirship plugin. Reason: Configuration zip file not found under: " + path.join(configPath, prefZipFilename)
                );
            }
            ...
            if (!copyWithSuccess) {
                return reject(
                    "Failed to install UrbanAirship plugin. Reason: Unable to copy google services file to project: " + unzipedResourcesDir
                );
            }
        }else{
